...
/**
 * Klasse zum Entpacken von passwortgeschützten Zip-Dateien
 * Autor: Martin Matula
 * Quelle: http://blog.alutam.com/2009/10/31/reading-password-protected-zip-files-in-java/
 */
public class ZipDecryptInputStream extends InputStream {
    /**
     * Statische Methode zum Enpacken einer Zip-Datei.
     * 
     * Beispielaufruf: boolean success = ZipDecryptInputStream.DecryptZipFile("C:\\temp\\Test.zip","C:\\temp\\test\\","1230",true);
     * 
     * @return                  Gibt true zurück, wenn die Datei erfolgreich entpackt wurde (=Passwort richtig), false wenn nicht (=Passwort falsch oder ein anderer Fehler)
     * @param filename          Absoluter Dateipfad des zu extrahierenden Zip-Archivs. Z.B. C:\\temp\\Test.zip
     * @param extractDir        Pfad in den der Inhalt extrahiert werden soll. Z.B. C:\\temp\\test\\
     * @param EnableTextOutput  Unterdrückt Textausgaben wenn false.
     */
    public static boolean DecryptZipFile(String filename, String extractDir, String password, boolean EnableTextOutput)
    {
        try
        {
            ...
            ZipEntry ze;
                ...
                while ((ze = zis.getNextEntry()) != null) {
                    FileOutputStream fos = new FileOutputStream(extractDir + ze.getName());
                    int b;
                    while ((b = zis.read()) != -1) {
                        fos.write(b);
                    }
            ...
            zis.close();
            if (EnableTextOutput) System.out.println("Unzipped");
            return true;
        }
        
        catch(IOException e)
        {
            if (EnableTextOutput) System.out.println("Cannot unzip file: " + e);
            return false;
        }
    }
